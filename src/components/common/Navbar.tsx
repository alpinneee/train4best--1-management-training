...
// Define interface for user data state
interface UserDataState {
  name: string;
  role: string;
  email?: string; // Add email property to resolve linter error
}
                ...
                {isProfileOpen && (
                  <div className="absolute right-0 mt-3 w-48 bg-[#362d98] rounded-2xl shadow-lg overflow-hidden z-50 border border-white/10">
                    <div className="px-4 py-3 border-b border-indigo-600/30">
                      {/* Prioritize displaying the user's email if available, as it's often a more reliable identifier than a generic name. */}
                      {/* If email is not available, fall back to the name, then to 'Guest'. */}
                      <p className="font-semibold text-sm">
                        {userData?.email ?? userData?.name ?? "Guest"}
                      </p>
